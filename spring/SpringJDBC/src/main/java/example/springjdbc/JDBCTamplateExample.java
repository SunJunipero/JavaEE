...
public class JDBCTamplateExample {
    private static final Logger log = LoggerFactory.getLogger(JDBCTamplateExample.class);
...
//        manyToOne(jdbc);
    ...
    private static void queryForSimpleObject(JdbcTemplate jdbc) {
        ...
        int countOfRegion = jdbc.queryForObject("select count(*) from public.jc_region where region_name = ?", new String[]{"Moscow"}, Integer.class);
        log.debug("Region count: {}", countOfRegion);

        String regionName = jdbc.queryForObject("select region_name from public.jc_region where region_id = ? and region_name = ?", new Object[]{1L, "Moscow"}, String.class);
    ...
    private static void queryForComplexObject(JdbcTemplate jdbc) {
        ...
        Region region = jdbc.queryForObject("select region_id, region_name from public.jc_region where region_id = ? and region_name = ?", new Object[]{1L, "Moscow"}, new RowMapper<Region>() {
            @Override
    ...
    private static void insertNewRegion(JdbcTemplate jdbc) {
        ...
        PreparedStatementCreatorFactory creatorFactory = new PreparedStatementCreatorFactory("insert into public.jc_region (region_name) values (?)");
    ...
    private static void queryForListComplexObjects(JdbcTemplate jdbc) {
        ...
        List<Region> list = jdbc.query("SELECT region_id, region_name FROM public.jc_region",
    ...
    private static void batchUpdate(JdbcTemplate jdbc) {
        ...
        int[] ints = jdbc.batchUpdate("insert into public.jc_region (region_name) values (?)", new BatchPreparedStatementSetter() {
            @Override
    ...
    private static void insertConstructor(JdbcTemplate jdbc) {
        ...
        SimpleJdbcInsert insertActor = new SimpleJdbcInsert(jdbc).withSchemaName("public").withTableName("jc_region").usingGeneratedKeyColumns("region_id");
    ...
    private static void manyToOne(JdbcTemplate jdbc) {
        ...
        List<City> cityList = jdbc.query("select c.city_id as c_id, c.city_name as c_name, c.region_id as r_id, r.region_name as r_name from public.jc_city c inner join public.jc_region r on c.region_id=r.region_id", new RowMapper<City>() {
            @Override
    ...
    private static void oneToManyMultipleQueries(JdbcTemplate jdbc) {
        ...
        List<Region> regionList = jdbc.query("select r.region_id, r.region_name from public.jc_region r ", new RowMapper<Region>() {
            @Override
        ...
        for (final Region region : regionList) {
            List<City> cities = jdbc.query("select c.city_id, c.city_name from public.jc_city c where c.region_id = ?", new Integer[]{region.getRegionId()}, new RowMapper<City>() {
                @Override
                public City mapRow(ResultSet rs, int i) throws SQLException {
